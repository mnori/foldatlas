...
function fetch_raw() {
	base_sauce_url=$1
	base_filename=$2
	wget -nc "$base_sauce_url$base_filename"
}
...
function fetch_extract_gff3() {
	...
	then # if file does not exist...
		wget "$sauce_url.gff3.bz2" # fetch compressed file
...
# #################################################################################
# # the -nc "no-clobber" option prevents files that already exist from being overwritten.
...
wget -nc "ftp://ftp.arabidopsis.org/home/tair/Sequences/blast_datasets/TAIR10_blastsets/TAIR10_seq_20101214_updated"
...
# Get the chromosome sequences as well
